...
class FollowUpPatientsView(customtkinter.CTk):
    def __init__(self, parent_frame, user_id):
    # Titolo
        title = customtkinter.CTkLabel(parent_frame, text="Follow Up patients", font=("Arial", 20, "bold"), text_color="black")
    ...
    def get_follow_up_patients(self):
        conn = sqlite3.connect("Database_proj.db")
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM Follow_Up_Patients")
        rows = cursor.fetchall()
